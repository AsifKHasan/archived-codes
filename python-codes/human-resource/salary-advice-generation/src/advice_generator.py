...
class AdviceGenerator:
    ...
    def get_data(self, selection, json_path):
        ...
        salary = list(map(lambda v: {'wing' : v[4].strip(), 'unit' : v[5].strip(), 'name' : v[2].strip(), 'type' : v[6].strip().replace('-', ''), 'account' : v[9].strip(), 'payable' : v[61].strip().replace('-', ''), 'paythrough' : v[63].strip(), 'paystatus' : v[64].strip()}, vals[5:]))
